...
class ToolExecutor:
    ...
    def __init__(
        ...
        self._last_backup: Optional[Path] = None  # Track last created backup
    
    @property
    def last_backup_path(self) -> Optional[Path]:
        """Get the path to the last created backup.
        
        Returns:
            Path to last backup directory, or None if no backup was created
        """
        ...
        try:
            ...
            if self.enable_rollback:
                ...
                if self._current_backup:
                    backup_created = True
                    self._last_backup = self._current_backup  # Store for later access
